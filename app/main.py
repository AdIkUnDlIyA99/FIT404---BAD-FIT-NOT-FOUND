...
def section_header(title, size="lg"):
    """Render a split-colour section heading.

    size="lg" (default) matches the big editorial page headings.
    size="sm" renders a smaller sub-heading that still uses the same
    neon accent color, for headings like "Pieces / N" that sit below
    a larger primary heading on the same page.
    """
    words = title.split(" ", 1)
    accent = words[0]
    rest = words[1] if len(words) > 1 else ""
    size_class = " oc-section--sm" if size == "sm" else ""

    st.html(
        f"""
        <div class="oc-section{size_class}">
...
with st.sidebar:
    ...
    st.html(
        """
        <div class="oc-muted" style="text-align: center;">

            Copyright &copy; 2026 Aditya Kundliya.
            <br>
            All rights reserved.
...
# =============================================
    ...
    section_header(f"Pieces / {len(wardrobe)}", size="sm"
    )
