...
BACKGROUND_FILE = PROJECT_ROOT / "assets" / "fit404_background.png"
...
if CSS_FILE.exists():
    ...
    # Page background only. This does not modify sidebar/button/wardrobe CSS.
    if BACKGROUND_FILE.exists():
        background_b64 = base64.b64encode(BACKGROUND_FILE.read_bytes()).decode("ascii")
        st.markdown(
            f"""
            <style>
...
# =============================================
# ALWAYS-AVAILABLE SIDEBAR TOGGLE
# =============================================

def render_sidebar_toggle():
    # Independent of Streamlit's visible header/toolbar, so it cannot get lost.
    with st.container(key="fit404_sidebar_toggle"):
        st.iframe(
            r'''<!doctype html>
<html>
<head>
<meta charset="utf-8">
<style>
html, body { width:46px; height:46px; margin:0; padding:0; overflow:hidden; background:transparent; }
#fit404-sidebar-toggle {
    width:42px; height:42px; margin:2px; padding:0; display:flex;
    align-items:center; justify-content:center; cursor:pointer;
    border:1px solid rgba(200,255,50,.58); border-radius:11px;
    background:rgba(8,8,8,.94); box-shadow:0 7px 24px rgba(0,0,0,.42);
    transition:border-color .16s ease, background .16s ease, transform .16s ease;
}
#fit404-sidebar-toggle:hover { border-color:#c8ff32; background:rgba(200,255,50,.10); transform:translateY(-1px); }
#fit404-sidebar-toggle:active { transform:translateY(0) scale(.96); }
.menu-icon { width:20px; height:14px; display:flex; flex-direction:column; justify-content:space-between; }
.menu-icon span { display:block; width:100%; height:2px; border-radius:999px; background:#c8ff32; box-shadow:0 0 8px rgba(200,255,50,.20); }
</style>
</head>
<body>
<button id="fit404-sidebar-toggle" type="button" aria-label="Open or close navigation" title="Open / close navigation">
    <span class="menu-icon" aria-hidden="true"><span></span><span></span><span></span></span>
</button>
<script>
(function () {
    const button = document.getElementById("fit404-sidebar-toggle");
    function findNativeSidebarToggle() {
        ...
        const headerButtons = Array.from(doc.querySelectorAll('button[data-testid="stBaseButton-headerNoPadding"]'));
        return headerButtons.find((candidate) => {
...
def safe_text(value):
    """Escape dynamic/user-provided text before embedding it in st.html."""
...
<span class="fi-section-name">
    ...
    insight_map = {
        "Formal": ("Business / Formal", "Confident · Structured"),
        "Business Casual": ("Office / Interview", "Polished · Professional"),
        "Smart Casual": ("College / Dinner", "Polished · Relaxed"),
        "Streetwear": ("Casual / Social", "Urban · Expressive"),
        "Sporty": ("Active / Casual", "Dynamic · Comfortable"),
        "Minimal": ("Everyday / Smart Casual", "Clean · Understated"),
        "Preppy": ("College / Day Event", "Clean · Collegiate"),
        "Vintage": ("Casual / Social", "Retro · Characterful"),
        "Grunge": ("Casual / Social", "Relaxed · Alternative"),
        "Casual": ("Everyday", "Relaxed · Versatile"),
    }

    best_occasion, vibe = insight_map.get(
        style,
        ("Everyday", "Visual analysis active"),
    )

    overall_score = report.get(
        "overall_score",
        int((harmony + balance) / 2),
    )
...
with st.sidebar:
    ...
    # Custom button navigation instead of st.radio.
    # This avoids Streamlit's native radio indicators entirely.
    if "fit404_page" not in st.session_state:
        st.session_state.fit404_page = "Analyze Fit"

    nav_items = [
        ("Analyze Fit", "nav_analyze"),
        ("Fit Battle", "nav_battle"),
        ("Wardrobe", "nav_wardrobe"),
        ("Build Fit", "nav_build"),
    ]

    for nav_label, nav_key in nav_items:
        is_active = st.session_state.fit404_page == nav_label
        if st.button(
            nav_label,
            key=nav_key,
            width="stretch",
            ...
            if not is_active:
                st.session_state.fit404_page = nav_label
                st.rerun()

    page = st.session_state.fit404_page
    ...
    if file_a and file_b:
        ...
        st.html(
            ...
            <div class="oc-metric-grid">
                ...
                {metric_card("Look A Overall", f'{result["score_a"]}%')}
                {metric_card("Look B Overall", f'{result["score_b"]}%')}
                {metric_card("Look A Harmony", f'{a["color_harmony"]}%')}
                {metric_card("Look B Harmony", f'{b["color_harmony"]}%')}
                {metric_card("Look A Balance", f'{a["visual_balance"]}%')}
                {metric_card("Look B Balance", f'{b["visual_balance"]}%')}
...
# =============================================
    ...
    st.html(
        """
        <div class="oc-muted">
            FIT404 ranks combinations from your own wardrobe using occasion,
            color compatibility, weather suitability and style cohesion.
        </div>
        """
    )
    ...
    if not wardrobe:
        ...
        occasion_col, weather_col = st.columns(2, gap="medium")
        ...
        if st.button(
            ...
            if outfit:
                ...
                match_score = outfit.get("match_score", 0)
                breakdown = outfit.get("score_breakdown", {})

                st.html(
                    f"""
                    <div class="oc-style-card">
                        <div class="oc-style-label">FIT MATCH</div>
                        <div class="oc-style-name">{match_score}%</div>
                    </div>
                    """
                )
                ...
                st.html(
                    f"""
                    <div class="oc-metric-grid">
                        {metric_card("Occasion Match", f'{breakdown.get("occasion", 0):.0f}%')}
                        {metric_card("Color Match", f'{breakdown.get("color", 0):.0f}%')}
                        {metric_card("Weather Match", f'{breakdown.get("weather", 0):.0f}%')}
                        {metric_card("Style Cohesion", f'{breakdown.get("cohesion", 0):.0f}%')}
                    </div>
                    """
                )
